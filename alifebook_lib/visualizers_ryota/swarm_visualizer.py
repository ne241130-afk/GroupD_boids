...
class SwarmVisualizer(object):
    ...
    def update(self, position, direction, colors=None):
        ...
        if colors is not None:
            assert colors.ndim == 2 and colors.shape[0] == position.shape[0] and colors.shape[1] == 4
        ...
        if colors is not None:
            self._arrows.arrow_color = colors
